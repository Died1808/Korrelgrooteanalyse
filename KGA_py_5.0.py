...
out=pd.DataFrame(columns = [['nr','naam', 'x', 'y', 'diepte m NAP', 'D5','D10','D15','D17','D20','D25',
 ...
 'D95','D100', 'k_Sauerbrei','k_Pavchich','k_Seelheim','k_CarKoz','k_Alysen','k_Bedinger',
 'k_Terzaghi','k_Harleman','k_mediaan','k_mean']])
...
for k in range(1,lenmat,1):
    ...
    if (3 > df.iloc[1,1] > 0.1) and df.iloc[12,1] <5:
        Haazen = 250 * df.iloc[1,1]**2 # officieel 1000 maar dan ligt alles een facor 4 te hoog
    else:
        Haazen=np.nan
    ...
    if (1 > df.iloc[1,1] > 0.1) and df.iloc[12,1] <5:
        ...
        US=np.nan
    KCB    = Ck*(diwa12gr*g/Dynvis)*(Por**3/(1-Por)**2)*((df.iloc[1,1]**2))
    KCU    = (1/Vorm)*0.417*(Porget**3)/(1+Porget)*((df.iloc[10,1]**2)*((np.exp((-1/8)*np.log(df.iloc[14,1]/df.iloc[2,1]))**2))**2)*3600*24
    if(df.iloc[10,1] >2):
       CarKoz=np.nan
    else:
       CarKoz = (Por**3*df.iloc[10,1])/(36*(1-Por)**2)*3600*24 #WUkorr
    ...
    out.loc[k] = [k, naam, x, y, mid, *Dxwaarde, round(Sauerbrei,1),
